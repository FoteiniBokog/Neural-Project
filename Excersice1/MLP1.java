...
public class MLP1 {
	...
	public static int N = 4000; //N elements
	...
	public static int batches[] = {1,N/10,N/100,N}; 
	public static int neuronSets[][] = {{6,3},{7,4},{9,6}}; //number of neuron for every example
	...
	//Weights for each of the 3 layers
	...
	public static void main(String[] args) throws IOException {
		System.out.print("Start the program with two hidden layers!\n");
			...
			for (int j = 0; j < batches.length; j++) {
				for (int w = 0; w < 2; w++) {
					...
					System.out.println("\nchoose_func=" + choose_func + "\n H1 = " + H1 + " H2 = "
							+ H2 + " and with a mini  batch of : " + mini_batches + " elements.");
		...
		System.out.println("choose_func=" + best_choose_func + "\n with H1 = " + best_H1 + " H2 = "
				+ best_H2 + " and with a batch of : " + best_Batches + " elements.");
	...
	public static void test() {
		...
		for (int i = 0; i < N; i++) {
			...
			forwardPass(testData[i],d,arrayOut1,arrayOut2, forward_Output,K);//forwardpass
	...
	public static void backProp(double[] trainingData){
		...
		forwardPass(trainingData,d,arrayOut1,arrayOut2,forward_Output,K);
		
		// Calculate errors
	
		// error3 is error for the output layer
		for (int i = 0; i < K; i++) {
			if (forward_Output[K] != (i + 1)) {
				error3[i] = 0.0 - forward_Output[i];
			} else {
				error3[i] = 1.0 - forward_Output[i];
			}
		}
		for (int i = 0; i < K; i++) {
			totalError += Math.pow((error3[i]), 2) * 0.5;
		}
		...
		for (int i = 0; i < K; i++) {
			gradientOut[i] = error3[i] * derivativeL(forward_Output[i]) * learningRate;
			newBias3[i] += gradientOut[i];
		}
		...
		for (int i = 0; i < K; i++) {
			for (int j = 0; j < H2; j++) {
				...
				delta3[i][j] += gradientOut[i] * arrayOut2[j];
			}
		}
	...
	public static void forwardPass(double[] trainingData,int d,double[] arrayOut1,double[] arrayOut2,double[] forward_Output,int K) {
		...
		//*******************finalLayer(arrayOut2, fwpOutput);
		for (int i = 0; i < K; i++) {
			for (int j = 0; j < H2; j++) {
				forward_Output[i] += arrayOut2[j] * weights3[i][j];
			}
			forward_Output[i] += bias3[i];
	...
	public static double derTanh(double number) {
		return (1 - Math.pow(number, 2));
	}
	
	 //This function returns the derivative of logistic.
	public static double derivativeL(double number) {
